...
def title_check(title: str) -> bool:
    return title.find("Website | Your site") == 0
...
def collect_incidents_from_pagerduty(api_token: str,
    ...
    logging.info(
        "collect_incidents_from_pagerduty({},{},{})".format(start_date.isoformat(), end_date.isoformat(), offset))
    ...
    if response.status_code == 429:
        ...
        time.sleep(1)
        return collect_incidents_from_pagerduty(api_token, service_id, session, start_date, end_date, offset)
    ...
    if api_response["more"]:
        ...
        incidents.extend(
            collect_incidents_from_pagerduty(api_token, service_id, session, start_date, end_date, next_offset))
...
def parse_relativedelta(string: str) -> relativedelta:
    match = re.match(r"^(\d+) *(day|month|year)s?$", string)
    if match is None:
...
def main() -> int:
    ...
    argparser.add_argument("--api-token",
                           ...
                           type=str,
                           required=True,
                           help="https://support.pagerduty.com/docs/generating-api-keys"
                                "#section-generating-a-personal-rest-api-key")
    argparser.add_argument("--service-id",
                           dest="service_id",
                           type=parse_service_id,
                           required=True,
                           help="url (e.g. https://some.pagerduty.com/services/ABCDEF4) or service id (e.g. ABCDEF4)")
    argparser.add_argument("--incidents-since",
                           dest="incidents_since",
                           type=iso8601.parse_date,
                           required=True,
                           help="iso8601 date, e.g. 2019-01-01T00:00:00Z")
    argparser.add_argument("--incidents-until",
                           dest="incidents_until",
                           type=iso8601.parse_date,
                           required=True,
                           help="iso8601 date, e.g. 2019-11-30T23:59:59Z")
    argparser.add_argument("--report-step",
                           dest="report_step",
                           type=parse_relativedelta,
                           required=True,
                           help="e.g. 1 day, 1 month, 1 year")
    ...
    logging.debug("api_token={}...".format(args.api_token[:3]))
    logging.info("service_id={}".format(args.service_id))
    ...
    with requests.Session() as requests_session:
        ...
        for interval_since, interval_until in intervals_gen(args.incidents_since, args.incidents_until, collect_step):
            collected_incidents = collect_incidents_from_pagerduty(args.api_token, args.service_id,
                                                                   requests_session,
                                                                   interval_since, interval_until)
